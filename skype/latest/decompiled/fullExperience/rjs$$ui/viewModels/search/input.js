define("ui/viewModels/search/input", [
  ...
  function d() {
    ...
    e.searchStatus = t.observable("").extend({
      rateLimit: {
    ...
    e.init = function () {
      ...
      w = n.isFeatureOn(r.featureFlags.SKYPE_DIRECTORY_SEARCH);
      v = n.isFeatureOn(r.featureFlags.USE_BUSINESS_WORDING);
      d.subscribe(c.DIRECTORY_SEARCH_ACTIVATED, e.onDirectorySearchActivated);
      d.subscribe(c.DIRECTORY_SEARCH_DEACTIVATED, e.onDirectorySearchDeactivated);
      d.subscribe(c.DIRECTORY_SEARCH_RESULTS, e.onDirectorySearchResults);
      d.subscribe(c.GROUPS_SEARCH_RESULTS, _);
      d.subscribe(c.LOCAL_SEARCH_RESULTS, M);
      d.subscribe(c.RESET, e.onReset);
      d.subscribe(p.SIDEBAR_STATE_CHANGED, A);
      d.subscribe(c.QUERY_EXECUTED, T);
      d.subscribe(h.SWX_TIMELINE_LOADED, E);
      d.subscribe(h.SWX_ON_SIGN_OUT, S);
      m.inputFocus = e.hasFocus.subscribe(L);
      m.inputValue = e.query.subscribe(k);
      m.searchResults = e.query.subscribe(N);
      m.showSpinner = e.showSpinner.subscribe(D);
      e.showMoreResultsAvailableMessage = t.observable(!1);
      e.inputAriaLabel(u.fetch({ key: x("input_searchSkype_ariaLabel") }));
    };
    e.dispose = function () {
      d.unsubscribe(c.DIRECTORY_SEARCH_ACTIVATED, e.onDirectorySearchActivated);
      d.unsubscribe(c.DIRECTORY_SEARCH_DEACTIVATED, e.onDirectorySearchDeactivated);
      d.unsubscribe(c.DIRECTORY_SEARCH_RESULTS, e.onDirectorySearchResults);
      d.unsubscribe(c.GROUPS_SEARCH_RESULTS, _);
      d.unsubscribe(c.LOCAL_SEARCH_RESULTS, M);
      d.unsubscribe(c.RESET, e.onReset);
      d.unsubscribe(p.SIDEBAR_STATE_CHANGED, A);
      d.unsubscribe(c.QUERY_EXECUTED, T);
      d.unsubscribe(h.SWX_TIMELINE_LOADED, E);
      d.unsubscribe(h.SWX_ON_SIGN_OUT, S);
      m.inputFocus.dispose();
      m.inputValue.dispose();
      m.searchResults.dispose();
      m.showSpinner.dispose();
    };
    e.onDirectorySearchActivated = function () {
      e.directorySearchActive = !0;
      D(e.directorySearchActive);
    };
