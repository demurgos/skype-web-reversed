define("ui/viewModels/userSettings/userSettingsPageViewModel", [
  ...
  function C(e) {
    ...
    return t.filter(e, r).forEach(function (e) {
      ...
      g.isUnlistedCategory(r, i, y, b) && (s = t.find(n, { id: x.UNLISTED.id }));
      s.preferenceItems.push(E.build(e));
    }), n;
  }
  function k(e) {
  ...
  function _(e) {
    t.forEach(e, function (e) {
      this.registerDisposable(e.value.changed(O.bind(this, e)));
      this.registerDisposable(e.value.state.changed(M.bind(this, e.description.id)));
      e.value.get();
    }, this);
  }
  function D(e) {
    e.selectedCategory = t.head(e.categories) || {};
    F(e.selectedCategory.id);
  }
  function P(e, n) {
    var r = e.selectedCategory.id;
    e.selectedCategory = t.find(e.categories, { id: n }) || {};
    r !== e.selectedCategory.id && F(e.selectedCategory.id);
  }
  function H() {
    ...
    this.pageOpenTimeMark = u.build();
  }
  function B() {
    this.fragmentVisibility = !1;
    j(this.pageOpenTimeMark);
  ...
  function I(e) {
    this.secondaryScroll && this.unregisterDisposable(this.secondaryScroll);
    this.secondaryScroll = p.build(o.getElement(".UserSettingsPage-scroll-area", e));
    this.registerDisposable(this.secondaryScroll);
    this.secondaryScroll.init();
  }
  function q(e, n) {
    var i = d.resolve(s.serviceLocator.FEATURE_FLAGS), o, u = this;
    this.pageOpenTimeMark = null;
    this.state = m.overrideDefaults({}, this.getInitialState());
    this.params = m.overrideDefaults({}, this.getDefaultParams(), e);
    this.secondaryScroll = null;
    this.isAboutPageEnabled = i.isFeatureOn(s.featureFlags.ABOUT_PAGE_ENABLED);
    this.isAboutPageVisible = h.observable(!1);
    this.registerDisposable(this.state.selectedCategory.subscribe(function () {
      r.execute(function () {
        I.call(this, n.element);
      }, u, !0);
    }));
    this.model = { categories: C(this.params.preferences()) };
    L(this.model);
    i.isFeatureOn(s.featureFlags.SHOW_UNLISTED_SETTINGS) || A(this.model);
    D(this.model);
    o = t(this.model.categories).map(function (e) {
      return e.preferenceItems;
    }).flatten().map(function (e) {
      return e.id;
    }).sort().value();
    _.call(this, t.filter(this.params.preferences(), function (e) {
      return t.indexOf(o, e.description.id) >= 0;
    }));
    this.applyState(this.model);
    H.call(this);
    this.setContext(h.dataFor(n.element));
    this.registerEvent(s.events.navigation.FRAGMENT_SHOW, H, l.DIRECTION.ANY, this);
    this.registerEvent(s.events.navigation.FRAGMENT_HIDE, B, l.DIRECTION.ANY, this);
    d.resolve(s.serviceLocator.PUBSUB).publish(s.events.navigation.FRAGMENT_REMOVE_ALL_HIDDEN);
  }
