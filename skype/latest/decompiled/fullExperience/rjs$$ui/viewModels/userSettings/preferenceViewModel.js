define("ui/viewModels/userSettings/preferenceViewModel", [
  ...
  function p(e, t, h) {
    ...
    function m(t) {
      t = t || n.noop, p.toggleValue = function () {
        e.value.set(!e.value()), t();
      }, p.activate = function () {
        p.toggleValue();
      };
    }
    ...
    p.id = e.description.id, p.uiState = r.preferenceValueState.Unknown, p.value = null, p.ariaId = "userSettings-" + p.id, p.template = a.getPrefTemplate(e.description, t), p.title = a.getPrefTitle(p.id, t), p.secondaryText = a.getPrefSecondaryText(p.id, t);
    switch (d) {
    ...
    case r.preferenceType.AutoBuddyDiscoveryPhone:
      p.showToggle = !0, p.showRemoveLink = !0, p.remove = function () {
        l.resolve(i.serviceLocator.ACTION_TELEMETRY).recordAction(c.shortCircuit.deleteNumber), e.value.set(null).then(function () {
          h.value.get();
        });
  ...
  t.Preference = p, t.build = function (e, t) {
    return new p(e, h, t);
  };
})
