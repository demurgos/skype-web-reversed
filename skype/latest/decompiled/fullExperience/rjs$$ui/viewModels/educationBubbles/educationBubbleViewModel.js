define("ui/viewModels/educationBubbles/educationBubbleViewModel", [
  ...
  function h(e, h, p, d, v, m) {
    ...
    function P() {
      ...
      b.bottom(e + t.ARROW_HEIGHT_IN_PX + c);
      b.arrowBottom(e + c);
      b.orientation(r.educationBubbles.ORIENTATION.TOP);
    }
    function H() {
      var e = Math.floor(h.getBoundingClientRect().bottom - p.getBoundingClientRect().top);
      b.top(e + t.ARROW_HEIGHT_IN_PX + c);
      b.arrowTop(e + c);
      b.orientation(r.educationBubbles.ORIENTATION.BOTTOM);
    }
    function B() {
      var e = Math.floor(h.getBoundingClientRect().bottom - p.getBoundingClientRect().top - h.getBoundingClientRect().height / 2);
      b.top(e - t.BUBBLE_MIN_HEIGH_IN_PX / 2 + c);
      b.arrowTop(e + c);
    }
    function j() {
      ...
      b.left(Math.floor(u) + c);
      b.right(l);
      b.arrowLeft(Math.floor(r - t.ARROW_WIDTH_IN_PX / 2) + c);
      b.arrowRight(l);
    }
    function F() {
      var e = t.BUBBLE_WIDTH_IN_PX + t.ARROW_HEIGHT_IN_PX, n = h.getBoundingClientRect();
      b.left(n.left - e + c);
      b.right(l);
      b.arrowLeft(Math.floor(n.left - t.ARROW_HEIGHT_IN_PX / 2 - t.ARROW_WIDTH_IN_PX / 2) + c);
      b.arrowRight(l);
      b.orientation(r.educationBubbles.ORIENTATION.LEFT);
    }
    function I() {
      var e = h.getBoundingClientRect();
      b.left(e.right + t.ARROW_HEIGHT_IN_PX + c);
      b.right(l);
      b.arrowLeft(Math.floor(e.right + t.ARROW_HEIGHT_IN_PX / 2 - t.ARROW_WIDTH_IN_PX / 2) + c);
      b.arrowRight(l);
      b.orientation(r.educationBubbles.ORIENTATION.RIGHT);
    }
    function q(e) {
    ...
    function R() {
      i.removeEventListener(r.events.browser.RESIZE, E);
      w.unsubscribe(r.events.narrowMode.SIDEBAR_STATE_CHANGED, S);
      g.dispose();
      y && y.dispose();
    }
    ...
    b.text = d;
    b.iconUrl = v;
    b.hidden = u.observable(!1);
    b.left = u.observable();
    b.right = u.observable();
    b.top = u.observable();
    b.bottom = u.observable();
    b.arrowLeft = u.observable();
    b.arrowRight = u.observable();
    b.arrowTop = u.observable();
    b.arrowBottom = u.observable();
    b.orientation = u.observable();
    g = b.hidden.subscribe(R);
    x();
    T();
    i.addEventListener(r.events.browser.RESIZE, E);
    w.subscribe(r.events.narrowMode.SIDEBAR_STATE_CHANGED, S);
    m.mutatingContainerElementQuery && q(m.mutatingContainerElementQuery);
  }
  ...
  t.OUTER_SPACING_IN_PX = 15;
  t.ARROW_WIDTH_IN_PX = 35;
  t.ARROW_HEIGHT_IN_PX = 9;
  t.BUBBLE_WIDTH_IN_PX = 288;
  t.BUBBLE_MIN_HEIGH_IN_PX = 68;
