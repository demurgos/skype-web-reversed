    ...
    h.call(e), e.searchResults = t.observableArray(), e.selectedContacts = t.observableArray(), e.state = t.observable(v.STATE_DEFAULT), e.hasSearchList = t.computed(k), e.hasDefaultList = t.computed(L), e.isVisible = t.computed(A), e.css = t.computed(y), e.spacesEnabled = N.isFeatureOn(u.featureFlags.SPACES), e.selectedItemTextDetails = t.observable(""), e.init = function (n, r) {
      return e.showJumpToConversation = n.showJumpToConversation, y(n.className), t.isObservable(n.conversation.participants) ? e.exclusionList = O() : (x = n.conversation.participants, C = x.subscribe(), x.changed(_)), e.registerEvent(m.ROSTER_QUERY_CHANGED, function (t) {
        ...
        if (t && t.person) {
          ...
          S(n), e.selectedContacts.remove(n), c.deselectContactFromDefaultList(n.getPerson(), e);
        }
      }), h.prototype.init.call(e, n, r);
    }, e.dispose = function () {
      o.disposeAndClearArray(e.searchResults), o.disposeAndClearArray(e.selectedContacts), e.hasSearchList.dispose(), e.hasDefaultList.dispose(), e.isVisible.dispose(), e.css.dispose(), C !== null && (C.dispose(), C = null, x.changed.off(_), x = null), h.prototype.dispose.call(e);
      ...
      return l.limit(a.searchLimit), l.sources(s.searchScope.AddressBook), l.text(t), h = l.getMore().then(function () {
        ...
        T = null, e.state(v.STATE_SEARCH), n.mark(g.SEARCH.LOCAL.END), o.disposeAndClearArray(e.searchResults), e.contactsCount(t.length), e.searchResults(t), e.dispatchEvent(m.ROSTER_QUERY_EXECUTED, { moreResultsAvailable: l.moreResultsAvailable() });
      }), T = h, T;
    }, e.onContactSelected = function (t, n) {
      n.preventDefault(), S(t), t.isActive() ? b(t, e.selectedContacts()) || (e.selectedContacts.push(t), e.dispatchEvent(m.PICKER_CONTACT_SELECTED, t.getPerson()), c.selectContactFromDefaultList(t.getPerson(), e)) : (e.selectedContacts.remove(w(e.selectedContacts(), t.id())), e.dispatchEvent(m.PICKER_CONTACT_DESELECTED, t.getPerson()), c.deselectContactFromDefaultList(t.getPerson(), e)), e.state(v.STATE_DEFAULT);
    };
  }
  function b(e, t) {
