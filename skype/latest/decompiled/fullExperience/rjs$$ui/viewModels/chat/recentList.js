  ...
  return function (T, N, C, k) {
    ...
    function st(e) {
      if (!e)
        return;
      f.execute(function () {
        bt(I, e), St(e), q.update(), I = e;
      });
    }
    function ot() {
      v.mark(d.TIMELINE.SYNC_START), B.mePerson.displayName.get(), pt(), g.addEventListener("resize", pt);
    }
    function ut() {
      g.removeEventListener("resize", pt), P = !0, X = !1;
    }
    function at() {
      if (X)
        return;
    ...
    function vt() {
      ...
      var e = new k();
      e.publish(), l.publish(o.apiUIEvents.SWX_TIMELINE_LOADED, {}), X = !0;
    }
    function mt(e, t) {
      t._isFavorited && t._isFavorited.changed(yt);
      var n = new N(C);
      n.init(t), xt(n), p.insertAt(H(), e, n), q.update();
    }
    function gt(e) {
      var t = H()[e];
      t.separator.dispose(), t.separator = null, t.conversation._isFavorited && t.conversation._isFavorited.changed.off(yt), t.dispose(), p.removeFrom(H(), e), q.update();
    }
    function yt(e) {
      h.handleArrayChanges(H(), e, mt, gt), H.valueHasMutated(), q.update();
    }
    function bt(e, t) {
    ...
    function St(e) {
      if (!P || !e)
        return;
      P = !1, Et() && l.publish(u.OPEN_CONVERSATION, {
    ...
    L.init = function () {
      J(), q = b.build(document.querySelector(".swx .side .timeline .scrollingWrapper")), q.init(), z = document.querySelector(".swx .side .timeline .scrollingWrapper .scrollable"), W = document.querySelector(".swx .side .timeline .scrollingWrapper .scrollable div"), O = a.get().conversationsManager, B = a.get().personsAndGroupsManager, H = n.observableArray(), L.recentConversations = n.computed(it), L.currentCalls = n.pureComputed(Z), L.hasCurrentCall = function () {
        return L.currentCalls().length > 0;
      }, j = r.resolve(o.serviceLocator.NAVIGATION_CONTEXT), L.buttonsDisabled = T.buttonsDisabled, L.hasNotificationsToggle = $.isFeatureOn(o.featureFlags.HAS_TIMELINE_BADGE) && !$.isFeatureOn(o.featureFlags.SINGLE_CONVERSATION_MODE), L.hasBetaLabel = $.isFeatureOn(o.featureFlags.HAS_BETA_LABEL), L.hasFeedbackLink = $.isFeatureOn(o.featureFlags.HAS_FEEDBACK_LINK), L.hasFeedbackIcon = $.isFeatureOn(o.featureFlags.HAS_FEEDBACK_ICON), L.feedbackIconURL = E.appBaseUrl + "/assets/images/components/common/feedback.svg", F = !1, M = i.newObservableCollection(O.conversations), D = M.subscribe(yt, null, "arrayChange"), L.mePersonId = i.newObservableProperty(B.mePerson.id), K(), a.get().signInManager.state.when(s.loginState.SignedIn, ot), a.get().signInManager.state.when(s.loginState.SignedOut, ut), l.subscribe(o.events.system.DATE_CHANGED, J);
    }, L.dispose = function () {
      D.dispose(), q.dispose(), L.recentConversations.dispose(), L.mePersonId.dispose(), l.unsubscribe(o.events.system.DATE_CHANGED, J), g.removeEventListener("resize", pt);
