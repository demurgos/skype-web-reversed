define("ui/viewModels/chat/chatInput", [
  ...
  function h() {
    ...
    function g(t) {
      u.isFeatureOn(a.featureFlags.GIPHY) && (/^\/giphy\b/.test(t) ? x("giphy") : x(null));
    ...
    function w() {
      ...
      e.conversationModel.participants().forEach(function (e) {
        ...
        t.capabilities.chat.changed(b);
        h.push(r);
      });
    }
    function E() {
      h && (h.forEach(function (e) {
        e.person.capabilities.chat.changed.off(b);
    ...
    this.init = function (t, u) {
      ...
      this.isServiceEnabled = d() ? i.newObservableProperty(this.conversationModel.chatService.sendMessage.enabled) : n.observable(!0);
      this.hasChatCapability = n.observable(!0);
      d() && this.conversationModel.participants.changed(w);
      p = c.build(t.conversationModel);
      this.isEnabled = n.computed(S);
      ...
      this.registerEvent(s.textarea.CHANGED, g);
      this.registerEvent(s.textarea.SET_ATTRIBUTION, x);
      r.subscribe(s.expressionPicker.ITEM_SEND_REQUEST, y);
    ...
    this.dispose = function () {
      d() && (e.isEnabled.dispose(), e.isServiceEnabled.dispose(), E(), e.conversationModel.participants.changed.off(w));
      p.dispose();
      r.unsubscribe(s.expressionPicker.ITEM_SEND_REQUEST, y);
    };
