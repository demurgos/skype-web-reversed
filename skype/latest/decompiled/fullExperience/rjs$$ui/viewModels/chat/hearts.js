define("ui/viewModels/chat/hearts", [
  ...
  function c(e, t, c) {
    ...
    h.canHeart = u.observable(!1);
    h.heartsEnabled = u.observable(!1);
    h.heartsCount = u.observable(0);
    h.isHeartedByMe = u.observable(!1);
    h.usersHearts = u.observableArray();
    h.heartBar = u.observableArray();
    h.lovedByCount = u.computed(function () {
      return o.fetch({
    ...
    h._internal = {
      ...
      processUser: function (e) {
        var t = {};
        t.person = e.person;
        t.firstName = e.person.firstName();
        t.displayName = e.person.displayName();
        t.me = a.isMePerson(e.person);
        t.me && h.isHeartedByMe(!0);
        t.contact = i.build(e.person);
        h.usersHearts.push(t);
      },
      initUserHeartBar: function () {
        ...
        i = h.heartBar.indexOf(s);
        i > -1 && (h.heartBar.splice(i, 1), h.heartBar.unshift(s));
      },
      initEmotions: function () {
        ...
        if (t) {
          ...
          h.isHeartedByMe(!1);
          h.usersHearts.removeAll();
          h.heartBar.removeAll();
          n.forEach(h._internal.processUser);
          h._internal.initUserHeartBar();
          h.heartsCount(n.length);
        } else
    ...
    h.dispose = function () {
      h.hasHeartBar.dispose();
      h.lovedByCount.dispose();
      h._internal.deletedMsgSubscription && h._internal.deletedMsgSubscription.dispose();
      h._internal.emotionsObserver && h._internal.emotionsObserver.dispose();
    };
    h.heartMessage = function () {
      function a() {
        i.data.result = 200;
        i.setDuration(n.duration());
        i.publish();
      }
      function f(e) {
        h.isHeartedByMe(!h.isHeartedByMe());
        h.heartsCount(o);
        i.data.result = e.status;
        i.setDuration(n.duration());
        i.publish();
      }
      ...
      u && (o = u.users.length);
      i.data.annotationsCount = o;
      i.setParticipantCount(t.participants().length);
      i.setMessageType(e);
      h.isHeartedByMe(!h.isHeartedByMe());
      h.isHeartedByMe() ? (i.data.action = i.enums.ACTION_TYPE.SET, e.model.emotions.add({ key: r.emotionTypes.HEART }).then(a, f)) : (i.data.action = i.enums.ACTION_TYPE.UNSET, e.model.emotions.remove({ key: r.emotionTypes.HEART }).then(a, f));
    };
    h.hasHeartBar = u.computed(h._internal.onHeartBar);
    h.heartBarLoved = u.observable();
    h.heartsCount.subscribe(h._internal.onHeartBarLoved);
    h.triggerTooltip = function (e, t) {
      c({
        key: "hearts",
        eventPageY: t.pageY
      });
