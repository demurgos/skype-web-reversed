define("jSkype/modelHelpers/contacts/dataMappers/contactToPerson", [
  ...
  t.map = function (t, r, i) {
    ...
    r.displayName() === r.id() && u.mapDisplayNameFallback(a, m, r);
    i && r.status._set(o.getDefaultPresence(r, g));
    s.setAuthorization(r, g, f);
    t.blocked ? s.setBlocked(r, f) : s.setUnblocked(r, f);
    s.updateGroups(r);
  };
});
