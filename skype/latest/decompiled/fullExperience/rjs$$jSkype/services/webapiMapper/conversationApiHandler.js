define("jSkype/services/webapiMapper/conversationApiHandler", [
  ...
  return function (B, j) {
    function V(e) {
      F.isOnline(e === P.ONLINE);
      B._onlineStateChanged();
    }
    function $() {
      k.set(_.RESURECTION_KEY, "1");
    }
    function J(e) {
      e.eventMessages && W.eventMessages(e.eventMessages);
      e.actionName === "syncThread" && (z.handleThreadSync(e), y.updateNGCCallState(e.id, e.properties));
      e.actionName === "endpointCreation" && q.promise.state() === "pending" && q.resolve();
    }
    function K() {
      var e = l.get();
      e.subscribe("webapi:data", J);
      e.subscribe(D.FORCE_RESYNC, $);
      e.subscribe(D.ONLINE_STATE_CHANGED, V);
    }
    function Q(e, t) {
      var n = t.person;
      return {
    ...
    F.addConversationsCollectionTask = o.task();
    F.conversationsManager = B;
    F.isOnline = o.boolProperty(!0);
    F.init = function () {
      X = v.get();
      F.serviceLocator = c.build(b.settings.webApiServiceHost);
      F.webAPIService = p.getInstance(F.serviceLocator);
      K();
      F.startPolling();
      I.resolve();
    };
    ...
    F.createConversation = function (e, n, r) {
      ...
      C.execute(function () {
        ...
        n = t.filter(e.participants(), function (e) {
          var t = e.person;
          return !(a.isPstnId(t.id()) && t.id().length > M);
        });
        var s = n.map(Q.bind(null, O));
        s.push(Q("Admin", e.selfParticipant));
        F.webAPIService.createConversation(s, i, r);
      });
    ...
    F.sendMessage = function (e, t, n, r, i) {
      function s(t) {
        F.setConsumptionHorizonToMessage(e, n);
        f.publish(t.request.status);
        r();
      }
      function o(e) {
        f.publish(e.status);
    ...
    F.sendPollMessage = function (e, i, o, u) {
      ...
      S.html._set(E.content);
      F.sendMessage(e, E, S, function () {
        w.sendPoll(S);
        o();
      }, function () {
        w.error(e.conversationId, undefined, y, r.telemetry.poll.errorType.SEND);
        u();
      });
    };
    ...
    F.joinConversation = function (e, t) {
      ...
      return e._requestedJoining && e._requestedJoining.promise.state() === "pending" ? Promise.reject(new Error("already joining this conversation")) : (e._requestedJoining = o.task(), I.promise.then(function () {
        ...
        n = s.get().personsAndGroupsManager.mePerson;
        F.addParticipant(e.conversationId, n.id(), i.participantRole.Attendee, function () {
    ...
    F.setAlerts = function (t, n, r, i, s) {
      ...
      r || (r = "");
      F.webAPIService.setConversationOption(t, "alerts", n && r === "", u, s);
    };
    ...
    F.setConsumptionHorizonToMessage = function (e, t) {
      ...
      r = t.timestamp().getTime() + ";" + n + ";" + t.key();
      e._updateConsumptionHorizon(r);
      t instanceof u || F.setConsumptionHorizon(e);
    };
