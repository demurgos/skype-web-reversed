define("jSkype/services/calling/callingInitializer", [
  ...
  function v() {
    d.registerMessageHandler(f.build());
    d.registerMessageHandler(h.build());
  }
  ...
  t.postSignInInitialize = function () {
    function f(e) {
      ...
      n.get().conversationsManager._conversationsSynced.promise.then(o.initialize).then(null, function (e) {
        e === c.PLUGIN_ERRORS.REASON_ALREADY_EXISTS && u.disableCalling(a.callingNotSupportedReasons.PluginAlreadyRunning);
        r.getBrowserInfo().isShellApp && t.sendPluginErrorCommand(e);
      });
    }
    var t;
    i.get().isPluginlessCallingSupported() && l.initialize();
    r.getBrowserInfo().isShellApp && (t = p.init());
    s.settings.isPollingEnabled && d.start();
    n.get().personsAndGroupsManager.mePerson._initAVCapabilities();
    u.checkForCallingSupport(!1).then(f);
  };
  t.destroy = function () {
    s.settings.isPollingEnabled && d.stop();
    o.dispose();
  };
});
