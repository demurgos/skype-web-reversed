define("jSkype/models/file", [
  ...
  function c(e, t) {
    function h(e) {
      c.name._set(e.name);
      c.size._set(e.size);
      c.state._set(s.fileUploadState.Uploading);
    }
    var c = this;
    c._send = function (p) {
      function y() {
        v.status._set(s.activityStatus.Succeeded);
        c.state._set(s.fileUploadState.Sent);
        d.resolve();
        T(!0);
      }
      function b(e) {
        v.status._set(s.activityStatus.Failed);
        c.state._set(s.fileUploadState.Failed);
        d.reject(e);
        T(!1, e);
      }
      function w(e, t) {
        v.shouldAbort() && t.abort();
        e.lengthComputable && (v.progress(e.loaded / e.total * 100), c.progress._set(v.progress()));
      }
      function E(e) {
        c.icon._set(e);
        if (p.isImage()) {
      ...
      function S() {
        function n(t) {
          v = u.outgoingPictureActivityItem(e);
          e.historyService._addOutgoingMessage(v);
        ...
        function r(n) {
          v.documentId._set(n);
          t.sendPhotoSharingMessage(e, v, n, c.name(), y, b);
        }
        return { send: n };
      }
      function x() {
        function n(t) {
          ...
          if (!n) {
            ...
            v = u.outgoingTextMessageActivityItem({ content: s }, e);
            e.historyService._addOutgoingMessage(v);
            b("Feature is disabled");
            return;
          }
          v = u.outgoingFileActivityItem(t, e);
          e.historyService._addOutgoingMessage(v);
        ...
        function r(n) {
          v.documentId._set(n);
          t.sendFileTransferMessage(e, v, n, c.name(), c.size(), y, b);
        }
        return { send: n };
      }
      function T(e, t) {
    ...
    c.name = r.property({ readOnly: !0 });
    c.size = r.property({
      readOnly: !0,
      value: 0
    });
    c.icon = r.property({ readOnly: !0 });
    c.progress = r.property({
      readOnly: !0,
      value: 0
    });
    c.state = r.property({ readOnly: !0 });
    c.cancel = r.disabledCommand();
    c.pause = r.disabledCommand();
    c.resume = r.disabledCommand();
  }
