define("jSkype/models/participant", [
  ...
  function f(e, f, l) {
    ...
    f.selfParticipant && f.selfParticipant.role.changed(m);
    c.person = e;
    c.chat = s.build(f);
    c.audio = r.build(f, e);
    c.video = i.build(f, e);
    c.screenSharing = o.build(f, e);
    c.lastReadTimestamp = t.property({
      readOnly: !0,
      value: null
    });
    c.state = t.property({
    ...
    c.chat.state.changed(d);
    c.audio.state.changed(d);
    c.video.state.changed(d);
    c.screenSharing.state.changed(d);
    e.id.changed(v);
    c._dispose = function () {
      c.chat.state.changed.off(d);
      c.audio.state.changed.off(d);
      c.video.state.changed.off(d);
      c.screenSharing.state.changed.off(d);
      f.selfParticipant.role.changed.off(m);
      c.chat._dispose();
      c.audio._dispose();
      c.video._dispose();
