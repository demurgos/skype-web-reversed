define("jSkype/models/videoChannel", [
  ...
  function u(e, u, a) {
    function g() {
      var t = !1;
      !d && !e.isGroupConversation() && (t = !0), h = r.createPropertyWithSetter(t, x, !0), c = t, p = h.set._enabled;
    }
    function y() {
    ...
    function S() {
      v.push(f.source.sink.container.changed(function () {
        y() ? w() : b() && E();
      })), v.push(f.state.when(s.mediaStreamState.Started, w)), v.push(f.state.when(s.mediaStreamState.Stopped, E)), m = u.when(s.callConnectionState.Disconnected, T);
    }
    function x(t) {
    ...
    function T(e, t) {
      if (t === undefined)
        return;
      f.state._set(s.mediaStreamState.Stopped), p(!0), h._set(!1);
    }
    var f = new i(), l = t.property({ value: !1 }), c, h, p, d = n.isMePerson(a), v = [], m;
    return g(), this._dispose = function () {
      f.state._set(s.mediaStreamState.Stopped), v.forEach(function (e) {
        e.dispose();
      }), v.length = 0, m.dispose();
