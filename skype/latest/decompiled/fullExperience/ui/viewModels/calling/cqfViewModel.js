  ...
  function h() {
    function y() {
      ...
      a.forEach(function (i) {
        i.checked = u.observable(!1), i.comment = u.observable(), i.qid && (e.callData.questionaryId = i.qid), t(i) && e.audioQuestions.push(i), n(i) && e.isPSTN && e.audioQuestions.push(i), r(i) && e.isVideoCall && e.videoQuestions.push(i);
      });
    }
    function b() {
    ...
    function S(e) {
      e.firstElementChild.classList.remove(m), e.firstElementChild.classList.add(v);
    }
    function x(e) {
      e.firstElementChild.classList.remove(v), e.firstElementChild.classList.add(m);
    }
    function T(e) {
    ...
    function k() {
      w(), r.get().cancel(e.callData), e.selectedStar(0);
    }
    var e = this, t, s, h = i.querySelectorAll(".CallQualityFeedback-starRating .btn"), p = "btn", d = "selected", v = "favouriteOn", m = "favouriteOff";
    e.currentStep = u.observable(1), e.selectedStar = u.observable(-1), e.canBeSubmitted = u.observable(!1), e.audioQuestions = [], e.videoQuestions = [], e.activitySubscriptions = [];
    var g = e.selectedStar.subscribe(function () {
      e.canBeSubmitted(!0), w();
    });
    e.init = function (r, i) {
      e.isVideoCall = r.callData.isVideo, e.isPSTN = r.callData.isPSTN, e.callData = r.callData, e.questionsTitle = u.computed(function () {
        return e.isVideoCall ? o.fetch({ key: "cqf_video_title" }) : o.fetch({ key: "cqf_audio_title" });
      }), t = f.build(i), t.init(), y(), b(), e.registerEvent(n.EVENTS.CQF_CANCEL, k);
      ...
      i && i.classList.remove(d), e.selectedStar(parseInt(r.getAttribute("data-value"))), r.classList.add(d), N();
    }, e.close = function () {
      k(), e.dispatchEvent(n.EVENTS.CQF_SCREEN_CLOSE, e.DIRECTION.PARENT);
    }, e.submit = function () {
      e.selectedStar() === 4 || e.currentStep() === 2 ? (e.callData.score = e.selectedStar(), e.callData.tokensSelected = [], e.callData.othersSelected = [], e.audioQuestions.concat(e.videoQuestions).forEach(function (t) {
        t.checked() && e.callData.tokensSelected.push({ token: t.token }), t.comment() && t.comment().trim().length > 1 && e.callData.othersSelected.push({
          token: t.token,
          value: t.comment()
        });
