define("ui/viewModels/calling/videoControlViewModel", [
  ...
  function c(e) {
    function m(e) {
      e.stream.state.changed(p), e.isStarted.set.enabled.changed(h);
    }
    function g() {
    ...
    e.participants.changed(g), t.call(c, e, h), c.isLocalVideoAvailable = n.computed(function () {
      return d() !== null;
      ...
      t.recordAction(u.audioVideo.toggleVideo), c.isLocalVideoOn() ? y(!1) : e.selfParticipant.video.state() === r.callConnectionState.Disconnected && e.videoService.start.enabled() ? e.videoService.start().then(function () {
        y(!0);
      }) : y(!0);
    }, c.dispose = function () {
      d.dispose(), c.videoStateText.dispose(), e.selfParticipant.video.channels.added.off(m), e.participants.changed.off(g);
