define("ui/viewModels/calling/skypeOut/skypeOutHeaderViewModel", [
  ...
  function S() {
    ...
    e.hasFocus = d.observable(!0), e.skypeOutInput = d.observable(""), e.isCallButtonDisabled = d.computed(function () {
      return a.activeCalls().length > 0 || e.skypeOutInput().length < 3;
    }), e.selectedCountry = d.observable(), e.init = function (r) {
      t = r, e.registerEvent(u.events.skypeOut.DIAL_BUTTON_CLICKED, e.insertAtCursor), n.subscribe(u.events.navigation.FRAGMENT_LOADED, C), c = e.skypeOutInput.subscribe(M), S = e.selectedCountry.subscribe(P), x = e.hasFocus.subscribe(O);
    }, e.insertAtCursor = function (n) {
      var r, i, s, o, u;
      i = D() ? T : t.selectionStart, s = D() ? T : t.selectionEnd, r = i, o = e.skypeOutInput(), u = g.inject(o, n.text, i, s), L(r + n.text.length), e.skypeOutInput(u);
    }, e.onKeyDown = function (t, n) {
      var r = l.getKeyCode(n);
      if (r === p.ESCAPE) {
        e.skypeOutInput() ? (e.resetSkypeOutInput(), C()) : k(), n.stopPropagation();
        return;
      }
      if (r === p.ENTER) {
        e.skypeOutInput() && !e.isCallButtonDisabled() && e.callPSTN(), n.stopPropagation();
        return;
      }
      return !0;
    }, e.callPSTN = function () {
      ...
      function d() {
        p(), v.initiatingPSTNCall(b.entryPoint.SKYPE_OUT_PAGE);
      }
      ...
      n.unsubscribe(u.events.navigation.FRAGMENT_LOADED, C), e.isCallButtonDisabled.dispose(), c.dispose(), S.dispose(), x.dispose();
  ...
  n.assign(S.prototype, c), t.build = function () {
    return new S();
  };
})
