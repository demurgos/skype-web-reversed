define("ui/viewModels/chat/headerNewConversation", [
  ...
  function h(e, t) {
    function g() {
      d(r.selectedContacts().length), t.participantsCountUpdated(d());
    }
    ...
    e.conversationModel ? p.conversationModel = e.conversationModel : p.conversationModel = {
    ...
    }, p.spacesEnabled = v.isFeatureOn(u.featureFlags.SPACES), p.telemetryContext = { source: l.invitesShareButtons.newConversation }, p.newConversationV2 = n.observable(v.isFeatureOn(u.featureFlags.NEW_CONVERSATION_V2)), p.showMoreActions = function () {
    ...
    }, m = n.observable(p.spacesEnabled), p.participantCount = n.pureComputed(function () {
      var e = d();
      ...
      e.setParticipantProvider(t), r = t, h = r.selectedContacts.subscribe(g);
    }, p.confirm = function () {
      p.dispatchEvent(c.newConversation.CONFIRMED, m()), m(p.spacesEnabled), t.publish();
    }, p.cancel = function () {
      p.dispatchEvent(c.newConversation.CANCELLED), m(p.spacesEnabled);
    }, p.forwardEvent(c.roster.ROSTER_QUERY_CHANGED), p.forwardEvent(c.roster.ROSTER_QUERY_EXECUTED), p.forwardEvent(c.roster.ROSTER_SELECTION_REMOVED), p.forwardEvent(c.roster.PICKER_CONTACT_SELECTED), p.forwardEvent(c.roster.PICKER_CONTACT_DESELECTED), p.forwardEvent(c.conversation.SHARED), p.registerEvent(c.conversation.JOINING_ENABLED, m), p.dispose = function () {
      h && h.dispose();
